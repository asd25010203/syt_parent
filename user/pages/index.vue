      ...
      <div class="left">
          ...
          <div>
            <div class="filter-wrapper">
              ...
              <div class="condition-wrapper">
                <span class="item v-link clickable" :class="hostypeActiveIndex == index ? 'selected' : ''" v-for="(item,index) in hostypeList" :key="item.id" @click="hostypeSelect(item.value, index)">{{ item.dictName }}</span>
              </div>
            </div>
            <div class="filter-wrapper">
              <span class="label">地区：</span>
              <div class="condition-wrapper">
                <span class="item v-link clickable" :class="provinceActiveIndex == index ? 'selected' : ''" v-for="(item,index) in districtList" :key="item.id" @click="districtSelect(item.value, index)">{{ item.dictName }}</span>
              </div>
            </div>
          </div>
        </div>
        <div class="v-scroll-list hospital-list">
          <div class="v-card clickable list-item" v-for="item in list" :key="item.id">
            <div class="">
              <div class="hospital-list-item hos-item" index="0" @click="show(item.hospCode)">
                <div class="wrapper">
                  <div class="hospital-title"> {{ item.hospName }}</div>
                ...
                <img :src="'data:image/jpeg;base64,'+item.picture" :alt="item.hospName" class="hospital-img">
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    querySearchAsync(queryString, cb) {
      ...
      hospitalApi.getByHosname(queryString).then(response => {
        for (let i = 0, len = response.data.length; i < len; i++) {
          response.data[i].value = response.data[i].hospName
        }
        cb(response.data)
      })
    },
    ...
    handleSelect(item) {
      window.location.href = '/hospital/' + item.hospCode
    },
    ...
    hostypeSelect(hostype, index) {
      ...
      this.searchObj.hospType = hostype
      this.getList();
    },
