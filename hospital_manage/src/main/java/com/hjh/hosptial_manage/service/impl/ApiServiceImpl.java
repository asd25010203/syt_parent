...
import com.hjh.hosptial_manage.mapper.ScheduleMapper;
import com.hjh.hosptial_manage.model.HospitalSet;
import com.hjh.hosptial_manage.model.Schedule;
import com.hjh.hosptial_manage.service.ApiService;
import com.hjh.hosptial_manage.utils.BeanUtils;
...
public class ApiServiceImpl implements ApiService {
    ...
    public JSONObject getHospital() {
        ...
        paramMap.put("hospCode",this.getHoscode());
    ...
    public boolean saveHospital(String data) {
        ...
        Map<String, Object> paramMap = new HashMap<>();
        paramMap.put("hospCode",this.getHoscode());
        paramMap.put("hospName",jsonObject.getString("hospName"));
        paramMap.put("hospType",jsonObject.getString("hospType"));
        ...
        //图片
        paramMap.put("picture", jsonObject.getString("picture"));
    ...
    public Map<String, Object> findDepartment(int pageNum, int pageSize) {
        ...
        paramMap.put("timestamp", HttpRequestHelper.getTimestamp());
        paramMap.put("sign", SecureUtil.md5(this.getSignKey()));
        JSONObject respone = HttpRequestHelper.sendRequest(paramMap,this.getApiUrl()+"/api/dep/department/list");
        if(null != respone && 200 == respone.getIntValue("code")) {
            JSONObject jsonObject = respone.getJSONObject("data");
    ...
    public boolean saveDepartment(String data) {
        ...
        for(int i=0, len=jsonArray.size(); i<len; i++) {
            ...
            paramMap.put("timestamp", HttpRequestHelper.getTimestamp());
            paramMap.put("sign",SecureUtil.md5(this.getSignKey()));
            JSONObject respone = HttpRequestHelper.sendRequest(paramMap,this.getApiUrl()+"/api/dep/saveDepartment");
            System.out.println(respone.toJSONString());
    ...
    public boolean removeDepartment(String depCode) {
        ...
        paramMap.put("timestamp", HttpRequestHelper.getTimestamp());
        paramMap.put("sign", SecureUtil.md5(this.getSignKey()));
        JSONObject respone = HttpRequestHelper.sendRequest(paramMap,this.getApiUrl()+"/api/dep/department/remove");
        System.out.println(respone.toJSONString());
        if(null != respone && 200 == respone.getIntValue("code")) {
            return true;
        } else {
    ...
    public Map<String, Object> findSchedule(int pageNum, int pageSize) {
        ...
        paramMap.put("timestamp", HttpRequestHelper.getTimestamp());
        paramMap.put("sign", SecureUtil.md5(this.getSignKey()));
        JSONObject respone = HttpRequestHelper.sendRequest(paramMap,this.getApiUrl()+"/api/Schedule/list");
        if(null != respone && 200 == respone.getIntValue("code")) {
            JSONObject jsonObject = respone.getJSONObject("data");
    ...
    public boolean saveSchedule(String data) {
        ...
        for(int i=0, len=jsonArray.size(); i<len; i++) {
            ...
            Map<String, Object> paramMap = new HashMap<>();
            paramMap.put("hospCode",schedule.getHoscode());
            ...
            paramMap.put("hosScheduleId",schedule.getId()+113);
            paramMap.put("timestamp", HttpRequestHelper.getTimestamp());
            paramMap.put("sign",SecureUtil.md5(this.getSignKey()));

            JSONObject respone = HttpRequestHelper.sendRequest(paramMap,this.getApiUrl()+"/api/Schedule/saveSchedule");
            System.out.println(respone.toJSONString());
            if(null == respone || 200 != respone.getIntValue("code")) {
                throw new HospitalException(respone.getString("message"), 201);
            }
    ...
    public boolean removeSchedule(String hosScheduleId) {
        ...
        paramMap.put("timestamp", HttpRequestHelper.getTimestamp());
        paramMap.put("sign", SecureUtil.md5(this.getSignKey()));
        JSONObject respone = HttpRequestHelper.sendRequest(paramMap,this.getApiUrl()+"/api/Schedule/removeSchedule");
        System.out.println(respone.toJSONString());
        if(null != respone && 200 == respone.getIntValue("code")) {
            return true;
        } else {
